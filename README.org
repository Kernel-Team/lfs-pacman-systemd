...
**** ~ninja~
     ...
     add_to_repo $PKGDEST/$pkgname-$pkgver-$pkgrel-$arch.pkg.tar.?z lfs-base
     #+END_SRC
From here we need to travel a bit and install lots of /python/ packages in order to have /setuptools/.
**** ~python2~
...
**** ~python-setuptools~
     ...
     ~/.local/bin/pip3 install pyparsing --user
...
**** TODO ~coreutils~
     got error:
     #+BEGIN_EXAMPLE :name coreutils-build-error
     help2man: can't get `--help' info from man/cp.td/cp
     Try `--no-discard-stderr' if option outputs to stderr
     make[2]: *** [Makefile:15302: man/cp.1] Error 127
